...
class SpeedPerturbation(object):
    """
    Performs Speed Augmentation by re-sampling the data, which does not preserve pitch.
    Args:
        min_speed_rate (float): Minimum speed rate.
        max_speed_rate (float): Maximum speed rate.
        p (float): Probability of applying the perturbation.
        rng: Random number generator.
    """

    def __init__(self, min_speed_rate=0.9, max_speed_rate=1.1, p=0.5, rng=None):
    ...
    def __call__(self, samples, sample_rate):
        if self._rng.random() < self.p:
            ...
            # Bỏ qua nếu tốc độ là 1.0
            if 0.99 < speed_rate < 1.01:
                return samples

            # Chuyển đổi numpy float32 sang pydub AudioSegment
            # pydub yêu cầu int16
            ...
            sound_with_altered_frame_rate = sound._spawn(sound.raw_data, overrides={
                "frame_rate": int(sound.frame_rate * speed_rate)
            })

            # Resample lại về sample rate gốc để hoàn tất việc thay đổi tốc độ
            final_sound = sound_with_altered_frame_rate.set_frame_rate(sound.frame_rate)

            # Chuyển đổi lại sang numpy float32
            return np.array(final_sound.get_array_of_samples(), dtype=np.float32) / 32767.0
...
def augment_clips(
        ...
        augmentation_probabilities: dict = {
            # Thêm xác suất cho SpeedPerturbation
            "Speed": 0.5,
        ...
        RIR_paths: List[str] = [],
        noise_type_probs: dict = None,
        save_debug_path: str = None,
        save_debug_count: int = 10
        ):
    """
    Applies audio augmentations... (docstring giữ nguyên)
    """

    # >>>>> THAY ĐỔI BẮT ĐẦU: Tạo thư mục debug và bộ đếm <<<<<
    if save_debug_path:
    ...
    # 0. Khởi tạo Speed Perturbation
    speed_augment = SpeedPerturbation(
        min_speed_rate=0.9, 
        max_speed_rate=1.2,
        p=augmentation_probabilities.get("Speed", 0.0) # Lấy giá trị từ dict
    )

    # --- Vòng lặp xử lý chính ---
    for i in range(0, len(clip_paths), batch_size):
        batch = clip_paths[i:i+batch_size]
        augmented_clips_tensors = []
        for clip in batch:
            # Tải clip
            ...
            # --- BƯỚC 0: SPEED PERTURBATION ---
            clip_data_np = speed_augment(clip_data_np, clip_sr)

            # --- Các bước xử lý còn lại ---
            if len(clip_data_np) > total_length:
                clip_data_np = clip_data_np[0:total_length]
            
            clip_data_np = create_fixed_size_clip(torch.from_numpy(clip_data_np), total_length, sr)

            # --- BƯỚC 1: Chuẩn hóa âm lượng RMS (sử dụng pydub) ---
            audio_segment = pydub.AudioSegment(
                (clip_data_np * 32767).astype(np.int16).tobytes(),
                frame_rate=sr, sample_width=2, channels=1
            )
            if random.random() < 0.6: 
                target_dBFS = random.uniform(-45.0, -30.0)
            else:
                target_dBFS = random.uniform(-30.0, -14.0)

            normalized_segment = match_target_amplitude(audio_segment, target_dBFS)
            normalized_samples_np = np.array(normalized_segment.get_array_of_samples(), dtype=np.float32) / 32767.0
            augmented_clips_tensors.append(torch.from_numpy(normalized_samples_np))

        # Gộp các clip đã xử lý thành một batch
        augmented_batch = torch.vstack(augmented_clips_tensors)
        device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
        
        # Chuyển batch lên GPU (nếu có)
        augmented_batch = augmented_batch.to(device)

        # --- BƯỚC 2: Thêm nhiễu nền (music, speech, noise) ---
        if np.random.random() < augmentation_probabilities.get("AddBackgroundNoise", 0.0):
            snr_ranges = {'music': (0, 15), 'speech': (-5, 10), 'noise': (5, 20)}
            noise_options = {}
            if music_paths: noise_options['music'] = music_paths
            if speech_paths: noise_options['speech'] = speech_paths
            if noise_paths: noise_options['noise'] = noise_paths

            if noise_options:
                # >>>>> THAY ĐỔI LOGIC CHỌN NHIỄU <<<<<
                
                # Lấy danh sách các loại nhiễu có sẵn
                available_noise_types = list(noise_options.keys())
                
                # Nếu có xác suất tùy chỉnh được cung cấp
                if noise_type_probs:
                    # Lọc ra các xác suất cho các loại nhiễu có sẵn
                    population = [nt for nt in noise_type_probs.keys() if nt in available_noise_types]
                    weights = [noise_type_probs[p] for p in population]

                    # Sanity check: nếu tổng khác 1, chuẩn hóa lại
                    if abs(sum(weights) - 1.0) > 1e-6:
                        logging.warning("Sum of noise_type_probabilities is not 1. Normalizing weights.")
                        total_weight = sum(weights)
                        weights = [w / total_weight for w in weights]
                    
                    # Chọn loại nhiễu theo trọng số
                    chosen_noise_type = np.random.choice(population, p=weights)
                
                else:
                    # Quay về logic cũ: chọn ngẫu nhiên đều
                ...
                noise_adder = torch_audiomentations.AddBackgroundNoise(
                    background_paths=chosen_paths, min_snr_in_db=min_snr,
        ...
        # --- BƯỚC 3: Thêm hiệu ứng vang (Reverb) ---
        ...
        # --- BƯỚC 4: Finalize và yield batch ---
        final_batch_np = (augmented_batch.cpu().numpy() * 32767).astype(np.int16)
        valid_indices = np.where(final_batch_np.max(axis=1) != 0)[0]
        if len(valid_indices) < final_batch_np.shape[0]:
            logging.warning(f"Removed {final_batch_np.shape[0] - len(valid_indices)} silent clips from batch.")
        
        final_batch_to_yield = final_batch_np[valid_indices]
        # >>>>> THAY ĐỔI BẮT ĐẦU: Logic lưu file debug <<<<<
        if save_debug_path and saved_count < save_debug_count:
            ...
            # Lưu một vài mẫu từ batch này
            num_to_save_this_batch = min(len(final_batch_to_yield), save_debug_count - saved_count)
            ...
            if saved_count >= save_debug_count:
                logging.info(f"Saved {saved_count} debug samples to '{save_debug_path}'. No more samples will be saved.")
                save_debug_path = None # Dừng việc lưu lại để không kiểm tra điều kiện này nữa
        # >>>>> THAY ĐỔI KẾT THÚC <<<<<
...
def generate_adversarial_texts(input_text: str, N: int, include_partial_phrase: float = 0, include_input_words: float = 0, all_positive_phrases: set = None):
    """
    ...
    # >>>>> THÊM VÀO ĐÂY <<<<<
    # Remove any matches that exist in the complete set of valid wake words
    if all_positive_phrases:
        adversarial_texts = [i for i in adversarial_texts if i not in all_positive_phrases]
    # >>>>> KẾT THÚC THÊM <<<<<
