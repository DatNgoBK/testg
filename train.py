...
def set_seed(seed=42):
    ...
    # PyTorch: đảm bảo tính nhất quán
...
class Model(nn.Module):
    def __init__(self, n_classes=1, input_shape=(16, 96), model_type="dnn",
        ...
        if model_type == "dnn":
            ...
            class SEBlock(nn.Module):
                ...
                def forward(self, x):
                    ...
                    return x * y.expand_as(x) # Nhân trọng số đã học vào các kênh
            # Kiến trúc CNN 1D đơn giản nhưng mạnh mẽ
            class CNNBlock(nn.Module):
                def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, dropout_p=0.1):
                    ...
                    self.se_block = SEBlock(out_channels) # <<<< THÊM VÀO ĐÂY
                    self.relu = nn.ReLU()
                ...
                def forward(self, x):
                    ...
                    x = self.norm(x)
                    x = x.transpose(1, 2)
                    x = self.se_block(x) # <<<< ÁP DỤNG Ở ĐÂY (sau Norm, trước ReLU)
                    x = self.relu(x)
                    x = self.pool(x)
                    x = self.dropout(x)
                    return x
            ...
            class Net(nn.Module):
                def __init__(self, input_shape, layer_dim=128, n_classes=1):
                    ...
                    # nn.Conv1d mong đợi (batch, channels, length)
                    # Ở đây, features sẽ là 'channels' và time_steps là 'length'
                    num_features = input_shape[1] # ví dụ: 96
                    
                    self.cnn_blocks = nn.Sequential(
                        # Block 1: 96 channels -> 64 channels, length giảm 1/2
                        CNNBlock(in_channels=num_features, out_channels=64, kernel_size=3),
                        # Block 2: 64 channels -> 128 channels, length giảm 1/2
                        CNNBlock(in_channels=64, out_channels=128, kernel_size=3),
                        # Block 3: 128 channels -> 128 channels, length giảm 1/2
                        CNNBlock(in_channels=128, out_channels=128, kernel_size=3)
                    )
                    
                    # Tính toán kích thước đầu ra sau các lớp CNN để đưa vào lớp Linear
                    # Đầu vào ban đầu: (batch, 96, 16)
                    # Sau block 1: (batch, 64, 8)
                    # Sau block 2: (batch, 128, 4)
                    # Sau block 3: (batch, 128, 2)
                    final_cnn_output_size = 128 * (input_shape[0] // (2**3)) # 128 * (16 // 8) = 128 * 2 = 256
                ...
                def forward(self, x):
                    # Đầu vào x có shape: (batch, time_steps, features) -> (batch, 16, 96)
                    
                    # Chuyển vị để khớp với định dạng của Conv1d: (batch, features, time_steps)
                    x = x.transpose(1, 2) # -> (batch, 96, 16)
                    
                    # Đưa qua các khối CNN
                    x = self.cnn_blocks(x) # -> (batch, 128, 2)
                    
                    # Làm phẳng để đưa vào lớp Linear
                    x = self.flatten(x) # -> (batch, 256)
                    
                    # Các lớp fully-connected cuối cùng
                    x = self.dense1(x)
                    x = self.relu(x)
                    x = self.dropout(x)
                    x = self.dense2(x)
            ...
            self.model = Net(input_shape, layer_dim=layer_dim, n_classes=n_classes)
        ...
        self.optimizer = optim.AdamW(self.model.parameters(), lr=0.0001, weight_decay=1e-4)
    ...
    def auto_train(self, X_train, X_val, false_positive_val_data, steps=50000, max_negative_weight=1000,
                   target_fp_per_hour=0.2, val_set_hrs=11.3,save_checkpoints_on_val=False, checkpoint_save_path=None):
        ...
        val_steps = list(range(10000, int(steps) + 1, 10000)) # <<<< DÒNG ĐÃ SỬA
        self.train_model(
                    X=X_train,
                    X_val=X_val,
                    ...
                    hold_steps=steps//3, lr=lr, val_set_hrs=val_set_hrs,save_checkpoints_on_val=save_checkpoints_on_val,checkpoint_save_path=checkpoint_save_path)
        
        if self.history.get("val_fp_per_hr") and max(self.history["val_fp_per_hr"]) < target_fp_per_hour:
            logging.info("#"*50 + "\nValidation FP rate is already excellent. Skipping fine-tuning sequences.\n" + "#"*50)
        # Chuyển thẳng đến bước hợp nhất mô hình
            pass # Lệnh pass sẽ làm nó nhảy qua 2 sequence tiếp theo và đến phần "Merge best models"
    ...
    def train_model(self, X, max_steps, warmup_steps, hold_steps, X_val=None,
                    ...
                    val_steps=[250], lr=0.0001, val_set_hrs=1,save_checkpoints_on_val=False, checkpoint_save_path=None):
        if save_checkpoints_on_val and checkpoint_save_path:
    ...
    # >>>>> KẾT THÚC THÊM <<<<<
        ...
        for step_ndx, data in tqdm(enumerate(X, 0), total=max_steps, desc="Training"):
            ...
            if step_ndx in val_steps and step_ndx > 1 and false_positive_val_data is not None:
                ...
                # >>>>> THÊM DÒNG NÀY <<<<<
                logging.info(f"\n===== Validation @ Step {step_ndx} =====")
                logging.info(f"Val FP per Hour: {val_fp_per_hr:.4f}")
                # >>>>> KẾT THÚC THÊM <<<<<
            ...
            if step_ndx in val_steps and step_ndx > 1 and X_val is not None:
                ...
                # >>>>> THÊM CÁC DÒNG NÀY <<<<<
                # In ra nếu chưa được in ở khối trên
                if 'val_fp_per_hr' not in self.history or len(self.history['val_fp_per_hr']) < len(self.history['val_accuracy']):
                    logging.info(f"\n===== Validation @ Step {step_ndx} =====")
                
                logging.info(f"Val Accuracy: {self.history['val_accuracy'][-1]:.4f}")
                logging.info(f"Val Recall: {self.history['val_recall'][-1]:.4f}")
                logging.info(f"===================================\n")
                # >>>>> KẾT THÚC THÊM <<<<<
                        # >>>>> THÊM LOGIC LƯU CHECKPOINT Ở CUỐI KHỐI VALIDATION <<<<<
                if save_checkpoints_on_val and checkpoint_save_path:
                    logging.info(f"Saving checkpoint at step {step_ndx}...")

                    # 1. Lưu mô hình PyTorch
                    # Đặt tên file rõ ràng
                    # pytorch_model_name = f"model_step_{step_ndx}.pt"
                    # pytorch_model_path = os.path.join(checkpoint_save_path, pytorch_model_name)
                    # torch.save(self.model, pytorch_model_path)
                    
                    # 2. Export ra ONNX
                    # Sử dụng hàm export_model đã có
                    # Lưu ý: hàm export_model của bạn có thể cần chỉnh sửa nhỏ nếu nó không nhận tên file đầy đủ
                    # Giả định export_model(model, model_name, output_dir)
                    onnx_model_name_base = f"model_step_{step_ndx}"
                    self.export_model(model=self.model, model_name=onnx_model_name_base, output_dir=checkpoint_save_path)
                    
                    logging.info(f"Checkpoint saved to {checkpoint_save_path}/{onnx_model_name_base}.onnx")
...
if __name__ == '__main__':
    ...
    # Get paths for different noise types
    noise_paths = {
        'music': [],
        'speech': [],
        'noise': []
    }
    for noise_type, paths in config["noise_paths"].items():
        duplication_rate = config["noise_paths_duplication_rate"].get(noise_type, 1)
        for path in paths:
            noise_paths[noise_type].extend([i.path for i in os.scandir(path)] * duplication_rate)
    ...
    if args.generate_clips is True:
        ...
        n_samples_to_generate = config["n_samples"] - n_current_samples
    ...
    # Đường dẫn đến file từ điển đã được lọc sẵn
            DICTIONARY_PATH = "filtered_common_words.txt"

    # Tải danh sách từ ngẫu nhiên từ file
            try:
                with open(DICTIONARY_PATH, 'r', encoding='utf-8') as f:
                # .read().splitlines() hiệu quả hơn readlines() vì nó loại bỏ ký tự '\n'
                    random_words = f.read().splitlines()
                logging.info(f"Successfully loaded {len(random_words)} words from {DICTIONARY_PATH}")
            except FileNotFoundError:
                logging.error(f"Dictionary file not found at {DICTIONARY_PATH}. Please run the preparation script.")
                # Hoặc có thể fallback về một danh sách mặc định nhỏ
                random_words = ["play", "stop", "weather", "time", "music"]
                logging.error(f"Falling back to a small default word list.")

    # ---- KẾT THÚC CHỈNH SỬA ----

    # Lấy danh sách các wake word từ config
            base_phrases = config["target_phrase"]

    # Phần còn lại của code giữ nguyên như trước...
            phrase_cycler = itertools.cycle(base_phrases)
            texts_to_generate = []
            PROB_ADD_RANDOM_WORD = 0.5

            for _ in range(n_samples_to_generate):
                current_phrase = next(phrase_cycler)
                if random.random() < PROB_ADD_RANDOM_WORD:
                    chosen_random_word = random.choice(random_words)
                    texts_to_generate.append(f"{current_phrase} {chosen_random_word}")
                else:
                    texts_to_generate.append(current_phrase)
            generate_samples(
                text=texts_to_generate, max_samples=config["n_samples"]-n_current_samples,
        ...
        if n_current_samples <= 0.95*config["n_samples_val"]:
            adversarial_texts = config["custom_negative_phrases"]
            all_positive_phrases_set = set(config["target_phrase"])
            for target_phrase in config["target_phrase"]:
                adversarial_texts.extend(generate_adversarial_texts(
                    input_text=target_phrase,
                    ...
                    include_partial_phrase=1.0,
                    include_input_words=0.2,
                    all_positive_phrases=all_positive_phrases_set))
    ...
    if args.augment_clips is True:
        if not os.path.exists(os.path.join(feature_save_dir, "negative_features_train1.npy")) or args.overwrite is True:
            aug_probs = config.get("augmentation_probabilities", {})
            noise_type_probs = config.get("noise_type_probabilities", None)
                    # Lấy đường dẫn debug từ config
            debug_path_positive = os.path.join(config.get("debug_augmentation_path", ""), "positive") if config.get("debug_augmentation_path") else None
            debug_path_negative = os.path.join(config.get("debug_augmentation_path", ""), "negative") if config.get("debug_augmentation_path") else None
            ...
            positive_clips_train_generator = augment_clips(positive_clips_train, total_length=config["total_length"],
                                                        ...
                                                        music_paths=noise_paths['music'],
                                                        speech_paths=noise_paths['speech'],
                                                        noise_paths=noise_paths['noise'],
                                                        RIR_paths=rir_paths,
                                                        augmentation_probabilities=aug_probs,
                                                        noise_type_probs=noise_type_probs,
                                                        save_debug_path=debug_path_positive)
            ...
            positive_clips_test_generator = augment_clips(positive_clips_test, total_length=config["total_length"],
                                                        ...
                                                        augmentation_probabilities=aug_probs,
                                                        noise_type_probs=noise_type_probs,
                                                        save_debug_path=debug_path_negative)
            ...
            #                                 output_file=os.path.join(feature_save_dir, "positive_features_train1.npy"),
    ...
    if args.train_model is True:
        ...
        X_train = torch.utils.data.DataLoader(IterDataset(batch_generator),
                                              batch_size=None, num_workers=16, prefetch_factor=16)

        X_val_fp = np.load(config["false_positive_validation_data_path"])

        num_original_fp_frames = X_val_fp.shape[0]
        X_val_fp = np.array([X_val_fp[i:i+input_shape[0]] for i in range(0, X_val_fp.shape[0]-input_shape[0], 1)])  # reshape to match model

        # --- TÍNH TOÁN THỜI LƯỢNG VALIDATION ĐỘNG ---
        HOP_LENGTH_SAMPLES = 1280
        SAMPLE_RATE = 16000

        # Tổng số mẫu audio = số frame * hop_length
        total_samples = num_original_fp_frames * HOP_LENGTH_SAMPLES
        # Tổng số giây = tổng số mẫu / sample_rate
        total_seconds = total_samples / SAMPLE_RATE
        # Tổng số giờ
        calculated_val_set_hrs = total_seconds / 3600

        logging.info(f"Dynamically calculated validation set duration: {calculated_val_set_hrs:.2f} hours")
        # --- KẾT THÚC TÍNH TOÁN ---
        ...
        best_model = oww.auto_train(
            ...
            val_set_hrs=calculated_val_set_hrs,
            save_checkpoints_on_val=config["save_checkpoints_on_val"],
